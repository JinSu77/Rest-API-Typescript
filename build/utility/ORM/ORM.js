...
class ORM {
     ...
     */
    static async GetAllActorsFromFilm(filmId) {
        const db = db_1.DB.Connection;
        ...
        WHERE fa.film_id = ?
      `;
        const [actors] = await db.query(query, [filmId]);
        return {
            page: 0,
            limit: actors.length,
            total: actors.length,
     ...
     */
    static async GetSingleActorFromFilm(filmId, actorId) {
        const db = db_1.DB.Connection;
        const columns = ['a.actor_id', 'a.first_name', 'a.last_name', 'a.last_update'];
        const query = `
      SELECT ${columns.join(',')}
    ...
    `;
        const [actors] = await db.query(query, [filmId, actorId]);
        if (actors.length === 0) {
            throw new ApiError_1.ApiError(ErrorCode_1.ErrorCode.BadRequest, 'sql/not-found', `L'acteur ${actorId} n'existe pas dans le film ${filmId}`);
        }
        return actors[0];
    }
}
